...
public class PaymentNotificationService {
    ...
    public void sendPaymentCreatedNotification(UUID bookingId, UUID paymentId, Long userId, BigDecimal amount) {
        ...
        notificationService.createNotification(userId, bookingId, paymentId, "Инструкция по бронированию", "Сумма проживания рассчитана: %s. Подтвердите намерение приехать. Оплата производится при заселении у администратора.".formatted(formatAmount(amount)), NotificationEventType.PAYMENT_INVOICE_CREATED);
    }
    ...
    public void sendPaymentPendingNotification(UUID bookingId, UUID paymentId, Long userId, BigDecimal amount) {
        ...
        notificationService.createNotification(userId, bookingId, paymentId, "Ожидается подтверждение", "Сумма проживания: %s. Нажмите подтверждение в боте, чтобы зафиксировать бронирование.".formatted(formatAmount(amount)), NotificationEventType.PAYMENT_PENDING);
    }
    ...
    public void sendPaymentCancelledNotification(UUID bookingId, UUID paymentId, Long userId, String reason) {
        ...
        notificationService.createNotification(userId, bookingId, paymentId, "Подтверждение отменено", "Подтверждение оплаты на месте было отменено.", NotificationEventType.PAYMENT_CANCELLED);
    }
    ...
    public void sendPaymentFailedNotification(UUID bookingId, UUID paymentId, Long userId, String reason) {
        ...
        notificationService.createNotification(userId, bookingId, paymentId, "Ошибка подтверждения", "Во время обработки подтверждения возникла ошибка.", NotificationEventType.PAYMENT_FAILED);
    }

    private String formatAmount(BigDecimal amount) {
        return amount == null ? "уточняется" : amount.stripTrailingZeros().toPlainString();
    }
}

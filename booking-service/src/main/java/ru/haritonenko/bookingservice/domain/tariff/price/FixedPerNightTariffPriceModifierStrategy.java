...
@Component
public class FixedPerNightTariffPriceModifierStrategy implements TariffPriceModifierStrategy {

    @Override
    public boolean supports(TariffPriceModifierType type) {
        return TariffPriceModifierType.FIXED_PER_NIGHT == type;
    }

    @Override
    public BigDecimal apply(BigDecimal basePrice, long nights, BookingTariffEntity tariff) {
        return basePrice.add(tariff.getPriceModifierValue().multiply(BigDecimal.valueOf(nights)));
    }
}
